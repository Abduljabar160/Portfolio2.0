        ...
        {EXPERIENCES.map((experience, index)=>(
            <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
             <div className="w-full lg:w-1/4 ">
                ...
                <a href="https://www.facebook.com/UNPTheOfficial"><h6 className="mb-2 font-semibold">
                    {experience.role} - <span className="text-sm text-purple-500">{experience.company}</span>
                </h6></a>
                <p className="mb-4 text-neutral-400">{experience.description}</p>
                {experience.technologies.map((tech, index)=>(
                    <span key={index} className="mr-2 mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-pink-400">{tech}</span>
                ))}
                </motion.div>
            </div>
        ))}
